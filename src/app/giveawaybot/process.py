...
def take_part_in_giveaway_from_record(tweet_record:dict):
    ...
    """    
    my_tw = tw()
    accounts_to_follow = tweet_record["mentionned_accounts_id"] + [tweet_record["accound_id"]]
    ...
    status["like"].append({tweet_record["tweet_id"]:my_tw.like_tweet(tweet_record["tweet_id"])})
    my_tw.retweet(tweet_record["tweet_id"])
    status["rt"].append({tweet_record["tweet_id"]:my_tw.retweet(tweet_record["tweet_id"])})
    return status    
